...
class Goalgen_Trainer(pl.LightningModule):
    ...
    def _initialize(self):
        self.model = IP2P(
            pretrained_model_dir=self.configs[
                "pretrained_model_dir"
            ],  # load the pretrained instructpix2pix weight
            encoder_dir=self.configs["encoder_dir"],
        ...
        if self.use_ema:
            ...
            requires_grad(
                self.ema_model, False
            )  # ema model will not be trained. It will only be updated.
            self.ema_model.eval()
    ...
    def from_checkpoint(cls, ckpt_dir=None, configs=None):
        if ckpt_dir is None:
            assert configs is not None, (
                "ckpt_dir and configs are both None for initialization."
            )
            return cls(configs)
    ...
    def on_save_checkpoint(self, checkpoint):
        if not self.use_ema:
            ...
            checkpoint["state_dict"] = {
                "unet_ema": self.ema_model.unet.state_dict(),
